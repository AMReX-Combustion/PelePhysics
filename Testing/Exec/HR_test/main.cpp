...
main(int argc, char* argv[])
{
...
#endif
  {
    ...
    //amrex::Real t0 = 700.0;
    //amrex::Real phi = 1.0;
    //amrex::Real p = 5.0*1013250.0;
    initializeData_HR(num_grow, mf, rY_source_ext, mfE, rY_source_energy_ext, t0, equiv_ratio, press, fctCount, dummyMask, finest_level, geoms, grids, dmaps, fuel_idx, ode_iE)
...
#pragma omp parallel
      ...
      for (amrex::MFIter mfi(mf[lev], tiling); mfi.isValid(); ++mfi) {
        ...
        if (reactFunc == 1) {
          doReact_ode_iE1(lev ,dt, ndt, omp_thread, mfi, mf, rY_source_ext, mfE, rY_source_energy_ext, fctCount, dummyMask, reactor);
