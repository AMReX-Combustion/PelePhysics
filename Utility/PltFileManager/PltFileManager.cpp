...
PltFileManager::readPlotFile()
{
  ...
  for(int lev = 0; lev < m_nlevels; ++lev)
  {
      ...
      VisMF::Read(m_data[lev], MultiFabFileFullPrefix(lev, m_pltFile, level_prefix, "Cell"));
  }
}

void
PltFileManager::readLevelBoxArray(int a_lev,
                                  BoxArray &a_grid)
{
    const std::string lvlHeader(m_pltFile + "/" + level_prefix + std::to_string(a_lev) +"/Cell_H");
...
void
PltFileManager::fillPatchFromPlt(int a_lev, const Geometry &a_level_geom, int pltComp,
                                 int dataComp, int nComp, MultiFab &a_mf)
{
    ...
    if (a_lev == 0) {
        ...
        AMREX_ALWAYS_ASSERT(m_geoms[0].ProbDomain().contains(a_level_geom.ProbDomain(),0.0000001));
        ...
        if (lev0rr == IntVect::TheUnitVector()) {
            PhysBCFunct<GpuBndryFuncFab<FillExtDirDummy>> bndry_func(a_level_geom, {dummyBCRec},
                                                                     FillExtDirDummy{});
            FillPatchSingleLevel(a_mf, IntVect(0), 0.0,
                                 {&(m_data[a_lev])},
                                 {0.0}, pltComp, dataComp, nComp, a_level_geom, bndry_func, 0);
        ...
        // Our level 0 is finer than the PltFile one.
            ...
            {
                PhysBCFunct<GpuBndryFuncFab<FillExtDirDummy>> crse_bndry_func(m_geoms[0], {dummyBCRec},
                                                                              FillExtDirDummy{});
                PhysBCFunct<GpuBndryFuncFab<FillExtDirDummy>> fine_bndry_func(a_level_geom, {dummyBCRec},
                                                                              FillExtDirDummy{});
                InterpFromCoarseLevel(a_mf, IntVect(0), 0.0,
                                      m_data[0], pltComp, dataComp, nComp,
                                      m_geoms[0], a_level_geom,
            ...
            for (int pltlev = 1; pltlev < m_geoms.size(); pltlev++) {
                ...
                IntVect rr  = a_level_geom.Domain().size() / m_geoms[pltlev].Domain().size();
                ...
                if (rr == IntVect::TheUnitVector()) {
                    ...
                    PhysBCFunct<GpuBndryFuncFab<FillExtDirDummy>> bndry_func(m_geoms[pltlev], {dummyBCRec},
                                                                             FillExtDirDummy{});
                    FillPatchSingleLevel(temp, IntVect(0), 0.0,
                                         {&m_data[pltlev]},
                                         {0.0}, pltComp, 0, nComp, m_geoms[pltlev], bndry_func, 0);
                ...
                // otherwise let just do another InterpFromCoarseLevel
                    ...
                    PhysBCFunct<GpuBndryFuncFab<FillExtDirDummy>> crse_bndry_func(m_geoms[pltlev], {dummyBCRec},
                                                                                  FillExtDirDummy{});
                    PhysBCFunct<GpuBndryFuncFab<FillExtDirDummy>> fine_bndry_func(a_level_geom, {dummyBCRec},
                                                                                  FillExtDirDummy{});
                    InterpFromCoarseLevel(temp, IntVect(0), 0.0,
                                          m_data[pltlev], pltComp, 0, nComp,
                                          m_geoms[pltlev], a_level_geom,
                                          crse_bndry_func, 0, fine_bndry_func, 0,
