...
# Parse arguments
#################################################################
parser = argparse.ArgumentParser(prog='Cantera PMF Generator',
...
parser.add_argument('-p',   '--pressure',    default=101325, type=float, help="Pressure [Pa]")
...
parser.add_argument('-o',   '--output',      default=None,               help=("Path to directory where flames will be saved, if not specified files are saved to"+
                                                                               "\nPelePhysics/Support/Mechanism/Models/<mechanism>/PMFs/"))
...
mech_paths = [name.strip() for name in open(os.path.join(pp_path, 'list_mech')).readlines()
...
qss_data = [name.split() for name in open(os.path.join(pp_path, 'list_qss_mech')).readlines()
            if not name.startswith('#')]
...
if mechanism in mech_names:
    ...
    raise RuntimeError ("Requested mechanism ("+mechanism+") found in neither list_mech or list_qssa_mech")
...
if mech_has_qssa:
    csv_file = os.path.join(outdir, str(label_pre +'-'+ label+"-X-qssa-removed.dat"))
    with open(csv_file, 'w') as outfile:
        writer = csv.writer(outfile, delimiter=' ', quotechar=' ', quoting=csv.QUOTE_MINIMAL)
        writer.writerow(['VARIABLES = "X" "temp" "u" "rho"']+['"X_'+x+'"' for x in gas.species_names if x in nonqss_spec_list])
        writer.writerow(['ZONE I=' + str(nz) + ' FORMAT=POINT' + ' SPECFORMAT=MOLE'])
        max_total_qss = 0.0
        for kk in range(len(f.grid)):
            f.set_gas_state(kk)
            zeroqss_mole_fracs = np.array([X if (gas.species_name(ii) in nonqss_spec_list) else 0.0 for ii,X in enumerate(gas.X) ])
            ...
            writer.writerow([f.grid[kk]*1e2,gas.T, f.velocity[kk]*1e2, gas.density*1e-3]
                            + [X for ii,X in enumerate(gas.X) if (gas.species_name(ii) in nonqss_spec_list)])
