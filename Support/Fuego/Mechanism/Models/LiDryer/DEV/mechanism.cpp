...
{
    std::cout << "in SPARSITY_INFO_SYST_SIMPLIFIED " << *consP << std::endl;
    amrex::Real c[9];
    amrex::Real J[100];
    ...
    std::cout << "in SPARSITY_INFO_SYST_SIMPLIFIED " << J[99] << std::endl;
...
AMREX_GPU_HOST_DEVICE void aJacobian_precond(amrex::Real *  J, amrex::Real *  sc, amrex::Real T, int HP)
{
    ...
    std::cout << "in aJacobian_precond " << mixture << std::endl;
    ...
    /* dTdot/dT */
    J[99] = -tmp1 + tmp2*dcmixdT - tmp3*dehmixdT;
    std::cout << "in aJacobian_precond " << J[99] << std::endl;
}
