...
from .RegularExpressions import eol, namedNumbers_3, whitespace, whitespaceOpt
...
class TemperatureRange(Token):
    ...
    pattern = (
        whitespaceOpt + namedNumbers_3 % _patternNames + whitespaceOpt + eol
    )
    ...
    def __str__(self):
        return "{temperature range: <%g><%g><%g>}" % (
            self.range[0],
            self.range[1],
            self.range[2],
        )
