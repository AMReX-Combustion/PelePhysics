...
class PythonPickler(PythonMill):
    ...
    def _rendermain(self):
        ...
        self._write(
            'print "Temperature from eytt is: %g (target ~ 1541.4 for GRImech)" % t'
        )
    ...
    def _renderwt(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "mwt[%d] = %f " % (species.id, species.weight)
    ...
    def _rendermmwy(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "YOW += y[%d]/%f " % (species.id, species.weight)
                + self.line("%s" % species.symbol)
            )
    ...
    def _rendermmwx(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "wtm += x[%d]*%f " % (species.id, species.weight)
                + self.line("%s" % species.symbol)
            )
    ...
    def _rendermmwc(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "W += c[%d]*%f " % (species.id, species.weight)
                + self.line("%s" % species.symbol)
            )
    ...
    def _renderxty(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "y[%d] = x[%d]*%f " % (species.id, species.id, species.weight)
                + self.line("%s" % species.symbol)
            )
    ...
    def _renderytx(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "x[%d] = y[%d]/%f " % (species.id, species.id, species.weight)
                + self.line("%s" % species.symbol)
            )
    ...
    def _renderxtcp(self, mechanism):
        ...
        self._write(
            "PORT = P/(%g * T) " % ((R * kelvin * mole / erg).value)
            + self.line("P/RT")
        )
        # now compute conversion
        self._write()
    ...
    def _renderytcp(self, mechanism):
        ...
        self._write(
            "PWORT = P/(YOW * %g * T) " % ((R * kelvin * mole / erg).value)
        )
    ...
    def _rendercty(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "y[%d] = c[%d]*%f " % (species.id, species.id, species.weight)
            )
    ...
    def _renderpx(self, mechanism):
        ...
        self._write(
            "P = rho * %g * T / XW " % ((R * kelvin * mole / erg).value)
    ...
    def _renderpy(self, mechanism):
        ...
        self._write(
            "P = rho * %g * T * YOW " % ((R * kelvin * mole / erg).value)
            + self.line("P = rho*R*T/W")
        )
    ...
    def _helmholtz(self, speciesInfo):
        ...
        self._generateThermoRoutine(
            "helmholtz", self._helmholtzNASA, speciesInfo
        )
    ...
    def _speciesEntropy(self, speciesInfo):
        ...
        self._generateThermoRoutine(
            "speciesEntropy", self._entropyNASA, speciesInfo
        )
    ...
    def _speciesInternalEnergy(self, speciesInfo):
        ...
        self._generateThermoRoutine(
            "speciesInternalEnergy", self._internalEnergy, speciesInfo
        )
    ...
    def _speciesEnthalpy(self, speciesInfo):
        ...
        self._generateThermoRoutine(
            "speciesEnthalpy", self._enthalpyNASA, speciesInfo
        )
    ...
    def _renderuml(self, mechanism):
        ...
        self._write(
            "RT = %15.8e*T " % ((R * kelvin * mole / erg).value)
            + self.line("R*T")
        )
    ...
    def _renderhml(self, mechanism):
        ...
        self._write()
        docstring = (
            "get enthalpy as a function of T for all species (molar units)"
        )
        self._write(self.line(docstring))
    ...
    def _forwardRate(self, mechanism, reaction):
        ...
        if sri:
            ...
            SRI = (
                "math.exp(X * math.log(%g*math.exp(%g/T) + math.exp(T/%g))"
                % (sri[0], -sri[1], -sri[2])
            )
            if len(sri) > 3:
    ...
    def _renderwc(self, mechanism):
        ...
        self._write()
        docstring = (
            "wdot = wc(T, conc): compute the production rate for each species"
        )
    ...
    def _renderqc(self, mechanism):
        ...
        self._write()
        docstring = (
            "qdot = qc(T,C): Returns the rate of progress for each reaction"
        )
    ...
    def _render_eytt(self, mechanism):
        ...
        self._write("ein  = e")
        self._write(
            "tmin = %g" % lowT + self.line("max lower bound for thermo def")
        )
        self._write(
            "tmax = %g" % highT + self.line("min upper bound for thermo def")
        )
    ...
    def _renderums(self, mechanism):
        ...
        self._write()
        docstring = (
            "ums = ums(T): Returns internal energy in mass units (Eq 30.)"
        )
        self._write(self.line(docstring))
        ...
        self._write(
            "RT = %g*T " % ((R * kelvin * mole / erg).value) + self.line("R*T")
        )
        ...
        for species in self.species:
            self._write(
                "ums[%d] *= RT/%f " % (species.id, species.weight)
                + self.line("%s" % species.symbol)
            )
    ...
    def _renderhms(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "hms[%d] *= RT/%f " % (species.id, species.weight)
                + self.line("%s" % species.symbol)
            )
    ...
    def _renderams(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "ams[%d] *= RT/%f " % (species.id, species.weight)
                + self.line("%s" % species.symbol)
            )
    ...
    def _rendergms(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "gms[%d] *= RT/%f " % (species.id, species.weight)
                + self.line("%s" % species.symbol)
            )
    ...
    def _rendersms(self, mechanism):
        ...
        self._write()
        docstring = (
            "sms = sms(T): Returns the entropies in mass units (Eq 28.)"
        )
        self._write(self.line(docstring))
    ...
    def _rendercpbl(self, mechanism):
        ...
        self._write()
        docstring = (
            "cpbl = cpbl(T,x): Returns the mean specific heat Cp (Eq. 33)"
        )
        self._write(self.line(docstring))
    ...
    def _rendercpbs(self, mechanism):
        ...
        self._write()
        docstring = (
            "cpbs = cpbs(T,y): Returns the mean specific heat Cp (Eq. 34)"
        )
        self._write(self.line(docstring))
    ...
    def _rendercvbl(self, mechanism):
        ...
        self._write()
        docstring = (
            "cvbl = cvbl(T,x): Returns the mean specific heat Cv (Eq. 35)"
        )
        self._write(self.line(docstring))
    ...
    def _rendercvbs(self, mechanism):
        ...
        self._write()
        docstring = (
            "cvbs = cvbs(T,y): Returns the mean specific heat Cv (Eq. 36)"
        )
        self._write(self.line(docstring))
    ...
    def _renderhbms(self, mechanism):
        ...
        self._write()
        docstring = (
            "hbms = hbms(T,y): Returns mean enthalpy of mixture in mass units"
        )
        self._write(self.line(docstring))
    ...
    def _rendersbml(self, mechanism):
        ...
        self._indent()
        self._write(
            "result += x[id]*(sor[id]-math.log((x[id]+%g))-logPratio)"
            % smallnum
        )
        self._outdent()
    ...
    def _rendersbms(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "result += x[%d]*(sor[%d]-math.log((x[%d]+%g))-logPratio) "
        ...
        self._write(
            "sbms = result * %g * YOW" % ((R * kelvin * mole / erg).value)
        )
    ...
    def _rendergbml(self, mechanism):
        ...
        self._write()
        docstring = (
            "gbml = gbml(P,T,x): Returns mean gibbs free energy in molar units"
        )
        self._write(self.line(docstring))
        ...
        self._indent()
        self._write(
            "result += x[id]*(gort[id]+math.log((x[id]+%g))+logPratio)"
            % smallnum
        )
        self._outdent()
    ...
    def _rendergbms(self, mechanism):
        ...
        self._write()
        docstring = (
            "gbms(P,T,y): Returns mixture gibbs free energy in mass units"
        )
        self._write(self.line(docstring))
        ...
        for species in self.species:
            self._write(
                "result += x[%d]*(gort[%d]+math.log((x[%d]+%g))+logPratio)"
                % (species.id, species.id, species.id, smallnum)
            )
    ...
    def _renderabml(self, mechanism):
        ...
        self._indent()
        self._write(
            "result += x[id]*(aort[id]+math.log((x[id]+%g))+logPratio)"
            % smallnum
        )
        self._outdent()
    ...
    def _renderabms(self, mechanism):
        ...
        for species in self.species:
            self._write(
                "result += x[%d]*(aort[%d]+math.log((x[%d]+%g))+logPratio)"
                % (species.id, species.id, species.id, smallnum)
            )
