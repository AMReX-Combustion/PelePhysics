...
class Parameters(BaseParser):
    ...
    def __init__(self, tokenizer):
        ...
        self._scanner = (
            fuego.serialization.chemkin.unpickle.scanners.parameters()
        )
    ...
    def _wrongCount(self, token, id, expectedCount):
        str = (
            "The %s reaction specification requires exactly %d parameters."
            % (id, expectedCount)
        )
        str = str + " '/%s/' apparently contains %d numbers" % (
            token.text,
            token.count,
        )
