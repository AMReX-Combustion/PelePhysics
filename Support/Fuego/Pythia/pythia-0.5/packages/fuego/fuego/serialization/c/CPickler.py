...
class CPickler(CMill):
    ...
    def _productionRate_GPU(self, mechanism):
        ...
        if nReactions == 0:
            ...
            self._write("const amrex::Real refCinv = 1.0 / refC;")
        ...
        if nReactions > 0:
            ...
            for i in range(nReactions):
                ...
                removeForward = self.removeForward(mechanism.reaction(id=i))

                if removeForward:
                    self._write("// Remove forward reaction")
                    self._write("//const amrex::Real qdot = qf - qr;")
                    self._write("const amrex::Real qdot = - qr;")
                else:
                    self._write("const amrex::Real qdot = qf - qr;")
    ...
    def _progressRate(self, mechanism):
        ...
        self._write(self.line("USES progressRate : todo switch to GPU"))
        self._write(
    ...
    def _QSScomponentFunctions(self, mechanism):
        ...
        #self._write()
        #self._write(
        #    "void comp_Kc_qss(amrex::Real invT, amrex::Real g_RT, amrex::Real g_RT_qss, amrex::Real * Kc)"
        #)
        #self._write("{")
        #self._indent()
