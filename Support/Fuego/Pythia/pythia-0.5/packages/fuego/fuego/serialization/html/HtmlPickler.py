...
class HtmlPickler(HTMLMill):
    ...
    def _body(self):
        if not self.__body:
            self._write(
                "<BODY aLink=#9933ff bgColor=#000000 link=#ffffff text=#ffffff vLink=#33eeff>"
            )
            self.__body = 1
        else:
            self._write("</BODY>")
    ...
    def _renderSpecies(self, mechanism):
        ...
        for species in mechanism.species():
            ...
            self._write(
                "<td width=80 bgcolor=#ffffff align=left height=30 vAlign=center><small>"
            )
            self._write('<font color="#000000">%d</font color></small>' % id)
            self._write(
                '<center> <a %s > <H2><font color="#0000FF"> %s</font color></H2> </a></center></td>'
                % (event, symbol)
            )
            self._outdent()
