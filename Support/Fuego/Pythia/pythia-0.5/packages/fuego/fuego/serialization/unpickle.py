...
def load(filename, format=None, mechanism=None):
    ...
    journal.debug("fuego").log(
        "loading file '%s', format='%s')" % (filename, format)
    )
    ...
    if not factory:
        journal.error("fuego").log(
            "unknown mechanism file format '%s'" % format
        )
        return None
...
def loadThermoDatabase(filename, format="chemkin", mechanism=None):
    ...
    from .chemkin.unpickle.parsers.ThermoDatabaseParser import (
        ThermoDatabaseParser,
    )
...
def locate(filename):
    ...
    if candidate:
        journal.debug("fuego").log(
            "resolved name '%s' as file '%s'" % (filename, candidate)
        )
    ...
    journal.error("fuego").log(
        "could not locate '%s' in %s" % (filename, pathlist)
    )
