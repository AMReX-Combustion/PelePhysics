...
class Mechanism(object):
    ...
    from .MechanismExceptions import (
        DuplicateElement,
        DuplicateQssSpecies,
        DuplicateSpecies,
        DuplicateThermalProperties,
    )
        ...
        for species in self._species.find():
            if not species.thermo:
                ...
                if not resolution:
                    ...
                    self._info.log(
                        "resolving species '%s' against '%s'"
                        % (species.symbol, self._thermdb)
                    )
    ...
    def _sort_reactions_within_type_tsp(self, n):
        ...
        troe_order = (
            self._reorder_reaction_set_tsp(rs[itroe[0] : itroe[1]]) + itroe[0]
        )
        sri_order = (
            self._reorder_reaction_set_tsp(rs[isri[0] : isri[1]]) + isri[0]
        )
        lind_order = (
            self._reorder_reaction_set_tsp(rs[ilindemann[0] : ilindemann[1]])
            + ilindemann[0]
        )
        thbody_order = (
            self._reorder_reaction_set_tsp(rs[i3body[0] : i3body[1]])
            + i3body[0]
        )
        simple_order = (
            self._reorder_reaction_set_tsp(rs[isimple[0] : isimple[1]])
            + isimple[0]
        )
        special_order = (
            self._reorder_reaction_set_tsp(rs[ispecial[0] : ispecial[1]])
            + ispecial[0]
        )
    ...
    def _tsp_solve(self, mat, improvement_threshold):
        ...
        def path_distance(r, c):
            return npy.sum(
                [
                    npy.linalg.norm(c[r[p]] - c[r[p - 1]], 1)
                ...
                for swap_first in range(
                    ...
                    for swap_last in range(
                        ...
                        if (
                            ...
                            route = (
                                new_route  # make this the accepted best route
                            )
