...
void
eval_single_point_transport(
  const std::vector<Real>& massFrac,
  const Real& temp,
  const Real& density,
  ...
  Real& conductivity)
{
  ...
  get_transport_coeffs(
    ivt, ivt, &(massFrac[0]), ivt, ivt, &temp, ivt, ivt, &density, ivt, ivt,
    &(mix_avg_diffusivity[0]), ivt, ivt, &shear_viscosity, ivt, ivt,
    &bulk_viscosity, ivt, ivt, &conductivity, ivt, ivt);
}
