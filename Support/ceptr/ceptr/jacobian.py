...
def dscqss_dsc_all(
    ...
    cw.writer(
        fstream,
        "AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE void "
        " dscqss_dsc_all(amrex::Real * dscqss_dsc, amrex::Real * sc, amrex::Real T)",
    )
    cw.writer(fstream, "{")
        ...
        if species_info.n_qssa_species > 0:
            ...
            for i in range(6):
                for j in range(6):
                    ...
                    dscqssdsc = syms.compute_dscqss_dsc(
                        scqss_idx=j, sc_idx=i, species_info=species_info
                    )
                    dscqss_dscList.append(dscqssdsc)
            ...
            try:
                ...
                print("Encountered recursion error, try without cse")
                syms.write_array_to_cpp_no_cse(
                    dscqss_dscList, f"dscqss_dsc", cw, fstream, indexList
                )
            cw.writer(fstream)
