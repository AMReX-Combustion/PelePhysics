...
def global_imw_str(idx):
    """Return a device aware string for imw."""
    return (
...
def ckpx(fstream, mechanism, species_info):
    ...
    for sp in species_info.nonqssa_species_list:
        ...
        cw.writer(
            fstream,
            f"XW += x[{spec_idx}]*{species.weight:f}; "
            + cw.comment(f"{species.name}"),
        )
...
def ckpc(fstream, mechanism, species_info):
    ...
    for sp in species_info.nonqssa_species_list:
        ...
        cw.writer(
            fstream,
            f"W += c[{spec_idx}]*{species.weight:f}; "
            + cw.comment(f"{species.name}"),
        )
...
def ckytx(fstream, mechanism, species_info):
    ...
    cw.writer(fstream, f"for (int i = 0; i < {n_species}; i++)")
    cw.writer(fstream, "{")
    cw.writer(fstream, "x[i] = y[i]*" + global_imw_str("i") + "*YOWINV;")
    cw.writer(fstream, "}")
...
def ckytcp(fstream, mechanism, species_info):
    ...
    cw.writer(fstream, f"for (int i = 0; i < {n_species}; i++)")
    cw.writer(fstream, "{")
    cw.writer(fstream, "c[i] = y[i]*" + global_imw_str("i") + ";")
    cw.writer(fstream, "}")
    cw.writer(fstream, f"for (int i = 0; i < {n_species}; i++)")
    cw.writer(fstream, "{")
    ...
    cw.writer(fstream, f"for (int i = 0; i < {n_species}; i++)")
    cw.writer(fstream, "{")
    cw.writer(fstream, "c[i] = PWORT * y[i] * " + global_imw_str("i") + ";")
    cw.writer(fstream, "}")
...
def ckytcr(fstream, mechanism, species_info):
    ...
    cw.writer(fstream, f"for (int i = 0; i < {n_species}; i++)")
    cw.writer(fstream, "{")
    cw.writer(fstream, "c[i] = rho  * y[i] * " + global_imw_str("i") + ";")
    cw.writer(fstream, "}")
    cw.writer(fstream, "}")
...
def ckums(fstream, mechanism, species_info):
    ...
    cw.writer(fstream, f"for (int i = 0; i < {n_species}; i++)")
    cw.writer(fstream, "{")
    cw.writer(fstream, "ums[i] *= RT*" + global_imw_str("i") + ";")
    cw.writer(fstream, "}")
    cw.writer(fstream, "}")
...
def ckhms(fstream, mechanism, species_info):
    ...
    cw.writer(fstream, f"for (int i = 0; i < {n_species}; i++)")
    cw.writer(fstream, "{")
    cw.writer(fstream, "hms[i] *= RT*" + global_imw_str("i") + ";")
    cw.writer(fstream, "}")
    cw.writer(fstream, "}")
...
def ckgms(fstream, mechanism, species_info):
    ...
    cw.writer(fstream, f"for (int i = 0; i < {n_species}; i++)")
    cw.writer(fstream, "{")
    cw.writer(fstream, "gms[i] *= RT*" + global_imw_str("i") + ";")
    cw.writer(fstream, "}")
    cw.writer(fstream, "}")
...
def ckams(fstream, mechanism, species_info):
    ...
    cw.writer(fstream, f"for (int i = 0; i < {n_species}; i++)")
    cw.writer(fstream, "{")
    cw.writer(fstream, "ams[i] *= RT*" + global_imw_str("i") + ";")
    cw.writer(fstream, "}")
    cw.writer(fstream, "}")
...
def ckwyr(fstream, mechanism, species_info):
    ...
    cw.writer(fstream, "{")
    cw.writer(fstream, "c[i] = 1e6 * rho * y[i]*" + global_imw_str("i") + ";")
    cw.writer(fstream, "}")
...
def ckchrgmass(fstream, species_info):
    ...
    cw.writer(
        fstream,
        f"zk[id] = {cc.Na:.8e} * {cc.qc:.8e} * kchrg[id] * "
        + global_imw_str("id")
        + ";",
    )
    cw.writer(fstream, "}")
