...
int reactor_init(const int* reactor_type, const int* Ncells,Real rk64_errtol,
    ...
    rhoX_init   = (Real *) malloc(data->ncells*sizeof(Real));
    rhoXsrc_ext = (Real *) malloc( data->ncells*sizeof(Real));
    rYsrc       = (Real *)  malloc((data->ncells*NUM_SPECIES)*sizeof(Real));
...
{
    Real time_init = time;
    Real time_out  = time + dt_react;
    Real current_time = time_init;
    Real *soln_reg,*carryover_reg,*error_reg,*zero_reg,*rhs;
    Real dt_rk,dt_rk_min,dt_rk_max,change_factor;
    const Real exp1=0.25;
    const Real exp2=0.2;
    const Real beta=1.0;
    const Real tinyval=1e-50;

    int neq_tot        = (NUM_SPECIES + 1);
    ...
    soln_reg      = (Real *) calloc(neq_tot,sizeof(Real));
    carryover_reg = (Real *) calloc(neq_tot,sizeof(Real));
    error_reg     = (Real *) calloc(neq_tot,sizeof(Real));
    zero_reg      = (Real *) calloc(neq_tot,sizeof(Real));
    rhs           = (Real *) calloc(neq_tot,sizeof(Real));
    ...
    dt_rk     = dt_react/Real(data->nsubsteps_guess);
    dt_rk_min = dt_react/Real(data->nsubsteps_max);
    dt_rk_max = dt_react/Real(data->nsubsteps_min);
    ...
    while(current_time < time_out)
    {
        current_time += dt_rk;
        nsteps++;
        std::memcpy(carryover_reg, soln_reg, sizeof(Real) * neq_tot);
        std::memcpy(error_reg,     zero_reg, sizeof(Real) * neq_tot);

        for(int stage=0;stage<nstages_rk64;stage++)
        {
            std::memcpy(rhs,zero_reg, sizeof(Real) * neq_tot);
    ...
    *time  = time_init + (*dt_react);
    ...
    std::memcpy(rY_in, soln_reg, neq_tot * sizeof(Real));
        ...
        Real &dt_react, Real &time)
{
    ...
    /* rhoY,T */
    std::memcpy(soln_reg,      rY_in, sizeof(Real) * neq_tot);
    std::memcpy(carryover_reg, rY_in, sizeof(Real) * neq_tot);
    /* rhoY_src_ext */
    std::memcpy(rYsrc, rY_src_in, (NUM_SPECIES * data->ncells)*sizeof(Real));
    /* rhoE/rhoH */
    std::memcpy(rhoX_init, rX_in, sizeof(Real) * data->ncells);
    std::memcpy(rhoXsrc_ext, rX_src_in, sizeof(Real) * data->ncells);
        ...
        void *user_data)
{
    ...
    for (tid = 0; tid < data_wk->ncells; tid ++) 
    {
        ...
        Real Xi[NUM_SPECIES];
        Real cdot[NUM_SPECIES], molecular_weight[NUM_SPECIES];
        Real cX;
        Real temp, energy;
        /* EOS object in cpp */
