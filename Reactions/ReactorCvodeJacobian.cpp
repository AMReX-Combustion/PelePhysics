  ...
  for (int tid = 0; tid < ncells; tid++) {
    ...
    for (int i = 0; i < NUM_SPECIES; i++) {
      ...
      for (int k = 0; k < NUM_SPECIES; k++) {
        J_col[offset + k] = Jmat_tmp[i * (NUM_SPECIES + 1) + k] *
#if AMREX_DEVICE_COMPILE
                            global_mw[k] * global_imw[i]
#else
                            h_global_mw[k] * h_global_imw[i]
#endif
          ;
      }
    ...
    for (int i = 0; i < NUM_SPECIES; i++) {
      J_col[offset + i] = Jmat_tmp[NUM_SPECIES * (NUM_SPECIES + 1) + i] *
#if AMREX_DEVICE_COMPILE
                          global_mw[i]
#else
                          h_global_mw[i]
...
#if AMREX_DEVICE_COMPILE
            global_mw[i] * global_imw[k]
#else
            h_global_mw[i] * h_global_imw[k]
#endif
            ;
        }
        Jmat_tmp[i * (NUM_SPECIES + 1) + NUM_SPECIES] *=
